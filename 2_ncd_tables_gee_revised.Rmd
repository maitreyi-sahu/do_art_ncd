...
for (v in unique(reg_vars$var_name)) {
  ...
  adj.m <- geeglm(get(v) ~ armR + site + age_cat + genderR + smokingR, 
              family = poisson, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  ...
  ext.m <- geeglm(get(v) ~ armR + site + age_cat + genderR + smokingR + exit_viral_load_suppressedR, 
              family = poisson, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
...
for (v in unique(lin_reg_vars$var_name)) {
  ...
  adj.lm <- geeglm(get(v) ~ armR + site + age_cat + genderR + smokingR, 
              family = gaussian, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  ...
  ext.lm <- geeglm(get(v) ~ armR + site + age_cat + genderR + smokingR + exit_viral_load_suppressedR, 
              family = gaussian, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
...
## Table 3: Cardiovascular risk for persons virally suppressed versus not suppressed at endline, South Africa (N = 983). 
&nbsp;
...
for (v in unique(reg_vars$var_name)) {
  ...
  int.m <- geeglm(get(v) ~ site + age_cat + genderR + smokingR + exit_viral_load_suppressed*armR, 
...
### Page Break
## Figure 1: Distribution of cardiovascular risk, by viral suppression status at endline [dashed lines are group medians]
