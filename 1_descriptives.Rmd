...
for (v in unique(reg_vars$var_name)) {
  ...
  m.rr <- exp(coef(m)[[2]])
  m.rr_ci <- exp(confint(m)["armCommunity arm", ])
  ...
  temp_row <- c(reg_vars[reg_vars$var_name == v, "var_label"], 
                ...
                paste0("(",
                       ...
                       ")"),
                as.numeric(round(m.p, round_digits)),
                adj.rr, adj.rr_ci, adj.p)
...
#              data = ncd_noHybrid,
#              family = quasibinomial(link = "log"))
#  adj.rr <- exp(coef(adj.m)[[2]])
#  adj.rr_ci <- exp(confint(adj.m)["armCommunity arm", ])
...
ft <- flextable(regressionDF) %>% 
  ...
  add_header_row(values = c("", "Baseline Analysis", "Adjusted Analysis"), top = T, colwidths = c(1, 3, 3)) %>% 
  align(i = 1, part = "header", align = "center") %>%
  theme_vanilla()
