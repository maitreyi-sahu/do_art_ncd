...
# Lines that change to add p-value
# 
# 	- Col_headings_freq
# 	- Col_headings_mean
# 
...
# 	    ,  p_value = character()
# 		
# 	- Insert line "Fisher's Exact Test"
# 	   # Fisher's Exact Test with simulated p-value
# 	    fisher.p.value = fisher.test(data[[v]], data$arm, simulate.p.value = T)$p.value
# 	    
# 	
# 	- Insert line 
# 	    %>%  mutate(p_value = round(fisher.p.value, round_p))

# 	- Insert line "ANOVA p-value"
# 	# Anova to compare means
# 	    anova.p.value = summary(aov(get(v) ~ arm, data = data))[[1]][["Pr(>F)"]][1]
# 	     
# 	 %>% mutate(p_value = round(anova.p.value, round_p))
# 	- Insert line 
# 	- In flextable, add:
# 		merge_v(j = "p_value")  %>%
#   -  width(width = 1.2) %>%
...
P-values are assessed as follows: 

+ For categorical variables, Fisher's Exact Test with simulated p-value (2000 replicates)
+ For continuous variables, Anova to assess difference in means
