...
&nbsp;
**Note that regression analysis uses smoking as a binary variable (even though descriptive shows categorical, because some of the regressions don't run otherwise - maybe getting small group problems)
&nbsp;
...
for (v in unique(reg_vars$var_name)) {
  ...
  adj.m <- geeglm(get(v) ~ armR + site + age_cat + genderR + smokingR + educationR, 
              family = poisson, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  ...
  ext.m <- geeglm(get(v) ~ armR + site + age_cat + genderR + smokingR + exit_viral_load_suppressedR + educationR, 
              family = poisson, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
...
ft <- flextable(regressionDF) %>% 
  ...
  add_header_row(values = c("", "Unadjusted Analysis", "Adjusted Analysis", "Extended Analysis (VS)"), 
...
for (v in unique(lin_reg_vars$var_name)) {
  ...
  adj.lm <- geeglm(get(v) ~ armR + site + age_cat + genderR + smokingR + educationR, 
              family = gaussian, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  ...
  ext.lm <- geeglm(get(v) ~ armR + site + age_cat + genderR + smokingR + exit_viral_load_suppressedR + educationR, 
              family = gaussian, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
...
+ Unadjusted model adjusts for site only
+ Adjusted models includes site, age, smoking and gender.
+ Extended model includes model exit viral suppression [binary] plus all adjustment variables
...
for (v in unique(reg_vars$var_name)) {
  ...
  adj.m <- geeglm(get(v) ~ site + age_cat + genderR + smokingR + exit_viral_load_suppressed + educationR, 
              family = poisson, data = vs_subset,
              id = hhid, corstr = "independence") 
  ...
  adj.m <- geeglm(get(v) ~ site + age_cat + genderR + exit_viral_load_suppressed + educationR, 
              family = poisson, data = vs_subset,
              id = hhid, corstr = "independence") 
...
for (v in unique(lin_reg_vars$var_name)) {
  ...
  adj.lm <- geeglm(get(v) ~  site + age_cat + genderR + smoking_status + exit_viral_load_suppressed + educationR, 
              family = gaussian, data = vs_subset,
              id = hhid, corstr = "independence") 
...
+ Unadjusted model adjusts for site only; adjusted models include site, age, smoking, and gender
...
for (v in unique(reg_vars$var_name)) {
  ...
  int.m <- geeglm(get(v) ~ site + age_cat + genderR + smokingR + exit_viral_load_suppressed*armR + educationR, 
              family = poisson, data = vs_subset,
              id = hhid, corstr = "independence") 
...
for (v in unique(lin_reg_vars$var_name)) {
  ...
  adj.lm <- geeglm(get(v) ~ exit_viral_load_suppressed + site + age_cat + genderR + smokingR + educationR, # NOTE THIS SMOKING VARIABLE IS DIFFERENT
...
+ Note, we drop 27 participants without exit viral load available.
+ Relative risk is estimated using a GEE poisson model (and log link).
+ Mean Difference is estimated using a GEE linear model.
+ Adjusted models includes site, age, smoking and gender.
