...
for (v in unique(reg_vars$var_name)) {
  ...
  m.n <- length(m$id) # number of observations

  # Adjusted for site, age, gender, smoking 
  
   if (v != "smokingR") {
  
  adj.m <- geeglm(get(v) ~ armR + site + age_catR + genderR + smokingR + educationR, 
              family = poisson, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
   }
  ...
  adj.m <- geeglm(get(v) ~ armR + site + age_catR + genderR + educationR, 
              family = poisson, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  ...
  adj.n <- length(adj.m$id) # number of observations
  
  # Adjusted for VS and site, age, gender, smoking 
  
  if (v != "smokingR") {
  
  ext.m <- geeglm(get(v) ~ armR + site + age_catR + genderR + smokingR + exit_viral_load_suppressedR + educationR, 
              family = poisson, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  }
  ...
  ext.m <- geeglm(get(v) ~ armR + site + age_catR + genderR + exit_viral_load_suppressedR + educationR, 
              family = poisson, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  ...
  ext.n <- length(ext.m$id) # number of observations
...
ft <- flextable(regressionDF) %>% 
  ...
  add_header_row(values = c("", "N", "Unadjusted Analysis", "Adjusted Analysis" 
...
for (v in unique(lin_reg_vars$var_name)) {
  ...
  rd_n <- length(lm$id) # number of observations
  
  # Adjusted for site, age, gender, smoking 
  
  adj.lm <- geeglm(get(v) ~ armR + site + age_catR + genderR + smokingR + educationR, 
              family = gaussian, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  ...
  adj.rd_n <- length(adj.lm$id) # number of observations
  
  # Adjusted for VS and site, age, gender, smoking 
  
  ext.lm <- geeglm(get(v) ~ armR + site + age_catR + genderR + smokingR + exit_viral_load_suppressedR + educationR, 
              family = gaussian, data = ncd_merged_subset,
              id = hhid, corstr = "independence") 
  ...
  ext.rd_n <- length(ext.lm$id) # number of observations
...
for (v in unique(reg_vars$var_name)) {
  ...
  adj.m <- geeglm(get(v) ~ site + age_catR + genderR + smokingR + exit_viral_load_suppressed + educationR, 
              family = poisson, data = vs_subset,
              id = hhid, corstr = "independence") 
  ...
  adj.m <- geeglm(get(v) ~ site + age_catR + genderR + exit_viral_load_suppressed + educationR, 
              family = poisson, data = vs_subset,
              id = hhid, corstr = "independence") 
...
for (v in unique(lin_reg_vars$var_name)) {
  ...
  adj.lm <- geeglm(get(v) ~  site + age_catR + genderR + smoking_status + exit_viral_load_suppressed + educationR, 
              family = gaussian, data = vs_subset,
              id = hhid, corstr = "independence") 
